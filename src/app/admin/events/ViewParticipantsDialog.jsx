...
const ViewParticipants = ({
  ...
  useEffect(() => {
    const fetchEventParticipants = async () => {
      if (!selectedEvent?.id) {
        ...
        setStaffFacultyParticipants([]);
        setCommunityParticipants([]);
        setLoading(false);
        return;
      }
      ...
      try {
        ...
        // Debug: Log all participants in STUDENTS collection to see what's there
        const allStudentsResponse = await getDocs(collection(db, COLLECTIONS.STUDENTS));
        console.log("All students in database:", allStudentsResponse.docs.map(doc => ({ id: doc.id, ...doc.data() })));
        console.log("Looking for eventId:", selectedEvent.id);

        // Store the fetched data in their respective state variables
        setEventParticipants(studentsResponse.docs.map(doc => ({ id: doc.id, ...doc.data() })));
        setStaffFacultyParticipants(staffResponse.docs.map(doc => ({ id: doc.id, ...doc.data() })));
        setCommunityParticipants(communityResponse.docs.map(doc => ({ id: doc.id, ...doc.data() })));

        console.log("Fetched participants:", {
          students: studentsResponse.docs.length,
          staff: staffResponse.docs.length,
          community: communityResponse.docs.length,
          total:
            studentsResponse.docs.length +
            staffResponse.docs.length +
            communityResponse.docs.length,
        });
      } catch (error) {
  ...
  }, [selectedEvent?.id]); // Use selectedEvent.id instead of selectedEvent object
